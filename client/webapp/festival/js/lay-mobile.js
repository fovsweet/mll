...
Layer.prototype.view = function(){
    ...
    if(!config.fixed){
     ...
     var laymshader = 'laymshade';
    if(config.type === 2){
        config.content = '<i></i><i class="laymloadtwo"></i><i></i>';
        laymshader = 'laymshade-tips'
    }
    
   
    
    layerbox.innerHTML = (config.shade ? '<div '+ (typeof config.shade === 'string' ? 'style="'+ config.shade +'"' : '') +' class="'+laymshader+'"></div>' : '')
